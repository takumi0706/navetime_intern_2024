...
const searchSpot = async () => {
    ...
    try {
        ...
        moveTime.value = await drawRoute(word.value, vias);
...
//Request Route
// APIを叩いてルートを取得する非同期関数
// const fetchRoute = async () => {
//     // ダミーAPI呼び出しを模倣
//     const response = await request('route_car', {
//         start: word.value,
//         goal: word.value,
//         via: `[${via.toString()}]`,
//     })
...
//     // APIレスポンスから移動時間を返す
//     return response.items[0].summary.move.time;
// }

// // ユーザーが入力したminutesと最も近いルートを検索する関数
// findClosestRoute = async (start, goal, vias, userMinutes) => {
...
//         try {
//             const time = await fetchRoute(start, goal, via);
